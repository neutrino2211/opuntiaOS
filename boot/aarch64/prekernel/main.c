...
static memory_map_t qemu_virt_memmap[2] = {
    {
        .startLo = 0x40000000, // 2GB
        .startHi = 0x0,
        .sizeLo = 0x80000, // 512KB
        .sizeHi = 0x0,
        .type = 0x1, // Free
        .acpi_3_0 = 0x0,
    },
    {
        .startLo = 0x40080000, // 2GB
        .startHi = 0x0,
        .sizeLo = 1000 << 20, // ~1GB
        .sizeHi = 0x0,
        .type = 0x1, // Free
...
#define memmap_size() sizeof(qemu_virt_memmap)
...
static int preserve_alloc_init(size_t kernsize)
{
    ...
    const size_t memmap = ROUND_CEIL(memmap_size(), page_size());
...
static memory_boot_desc_t memory_boot_desc_init()
{
    ...
    const size_t memory_layout_size = 0 + 1; // Including the trailing element.
    memory_layout_t* mem_layout_paddr = palloc_aligned(memory_layout_size * sizeof(memory_layout_t), page_size());

    // Init of trailing element.
    mem_layout_paddr[memory_layout_size - 1].flags = MEMORY_LAYOUT_FLAG_TERMINATE;
